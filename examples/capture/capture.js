...
$W = {
    ...
    $.each($W.markers, function(i, m) {
      $("#graph").append(
        '<div style="position:absolute;left:' +
          (m[2] + 4) +
          'px;top:10px;color:#aaa;font-size:smaller">' +
          m[0] +
          ": " +
  ...
  auto_detect_sample_row: function() {
    ...
    if ($("#autoDetectCanvas").length == 0) {
      $("body").append(
        "<canvas style='display:none;' id='autoDetectCanvas'></canvas>"
      );
    }
  ...
  notify: function(msg, type, expire) {
    ...
    $("#notify").html(
      $("#notify").html() + "<div id='notify_" + id + "' class='notify'></div>"
    );
    if (type == "warning")
      $("#notify_" + id)
        .html("<b>Warning:</b> " + msg)
        .addClass("warning");
    if (type == "error")
      $("#notify_" + id)
        .html("<b>Error:</b> " + msg)
  ...
  excerptCanvas: function(x1, y1, x2, y2, source) {
    ...
    if ($("#excerptCanvas").length == 0) {
      $("body").append(
        "<canvas style='display:none;' id='excerptCanvas'></canvas>"
      );
    }
